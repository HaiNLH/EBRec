...
def get_metrics(metrics, grd, pred, topks):
    ...
    for topk in topks:
        ...
        row_indice = torch.zeros_like(col_indice).to(pred.device) + torch.arange(pred.shape[0], device=pred.device, dtype=torch.long).view(-1, 1)
        is_hit = grd[row_indice.view(-1).to(pred.device), col_indice.view(-1).to(pred.device)].view(-1, topk).to(pred.device)
